...
*/
	...
	SVG.text_bbox = SVG.invent({
		...
		construct: {
			...
			text_bbox: function(title, font) {
				//var gr = new SVG.G();
				//this.put(gr);
				var txt = new SVG.Text(title).font({size:font['font-size'],family: font['font-family']}).move(0,0);
				this.put(txt);
				txt.text(title);
				//console.log(title);
				//gr.add(txt);
	...
	Raphael.fn.print_center = function(x, y, string, font, size, letter_spacing) {
		var txt = this.text(string).move(x, y).font({family: font,size: size});
		var bb = txt.bbox();
		...
		//m.translate(dx, dy);
		return txt.transform("t" + dx + "," + dy);
	};

	SVG.print_center = SVG.invent({
		create: 'text',
		inherit: SVG.Text,
		extend: {
		// Create method to proportionally scale the rounded corners
		...
		construct: {
			// Create a rounded element
			print_center: function(x, y, string, font, size, letter_spacing) {
				var txt = new SVG.Text(string).move(x, y).font({family: font,size: size});
				var bb = txt.bbox();
 ...
 ******************/
	...
	_.extend(BaseTheme.prototype, {
		init : function(diagram) {
			...
			l[LINETYPE.SOLID]  = '';
			l[LINETYPE.DOTTED] = '5,5';
		},

		init_paper : function(container) {
			this._paper = new SVG(container).size(320, 200);
			//this._paper.
		},
		...
		draw_rect : function(x, y, w, h) {
			return this._paper.rect(w, h).move(x, y);
		},
		...
		draw : function(container) {
			...
			this._paper.size(diagram.width, diagram.height);

			var y = DIAGRAM_MARGIN + title_height;

			set.add(this.draw_title());
			set.add(this.draw_actors(y));
			set.add(this.draw_signals(y + this._actors_height));
		...
		draw_self_signal : function(signal, offsetY) {
			...
			attr['marker-end'] = this.arrow_types[signal.arrowtype] + '-wide-long';
			line.attr(attr);
		},
		...
		draw_signal : function (signal, offsetY) {
			...
			line.attr(LINE);
			line.attr({
				'marker-end': this.arrow_types[signal.arrowtype] + '-wide-long',
				'stroke-dasharray': this.line_types[signal.linetype]
			});

			var ARROW_SIZE = 16;
			var dir = signal.actorA.x < signal.actorB.x ? 1 : -1;
			this.draw_arrowhead(bX, offsetY, ARROW_SIZE, dir);
		},
		...
		draw_text : function (x, y, text, font) {
			...
			if (f._obj) {
				...
				y = y-f['font-size']/2;
				t = paper.text(text).attr({'text-anchor':"middle"}).move(x,y);
				t.attr(f);
			}
			// draw a rect behind it
			var bb = t.bbox();
			var r = paper.rect(bb.width, bb.height).move(bb.x, bb.y);
		...
		draw_text_box : function (box, text, margin, padding, font) {
			...
			rect.attr(LINE).attr({'fill-opacity':0.01});
		...
		draw_arrowhead: function(x, y, size, direction) {
